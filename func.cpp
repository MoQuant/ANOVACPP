...
void ANOVA(std::vector<std::vector<double>> x, std::vector<std::vector<double>> y)
{
    ...
    std::cout << "Beta\tT-Stat\tPVal" << std::endl;
    for(int i = 0; i < tstat.size(); ++i){
        std::cout << round(beta[i][0]*1000)/1000 << "\t" << round(tstat[i]*1000)/1000 << "\t" << round(N(tstat[i])*1000)/1000 << std::endl;
    }
